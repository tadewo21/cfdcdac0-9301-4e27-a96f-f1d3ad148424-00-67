...
CREATE TABLE public.profiles (
  ...
  full_name TEXT,
  user_type TEXT NOT NULL DEFAULT 'job_seeker' CHECK (user_type IN ('job_seeker', 'employer', 'admin')),
...
USING (auth.uid() IN (
  SELECT user_id FROM public.profiles WHERE employer_id = employers.id
));
...
WITH CHECK (auth.uid() IN (
  SELECT user_id FROM public.profiles WHERE employer_id = jobs.employer_id
));
